...
export default function WelcomeMessage({ adminUser, onDismiss }: Props) {
  ...
  // FIX: onDismiss added to dependency array
  useEffect(() => {
    if (countdown <= 0) onDismiss();
  }, [countdown, onDismiss]);

  // FIX: memoize todayStart so it doesn't change on every render and invalidate the query
  ...
  return (
    ...
    >
      <div className="w-full max-w-md animate-in zoom-in-95 duration-500" style={{ fontFamily: "'DM Sans',sans-serif" }}>

        <div className="rounded-3xl overflow-hidden shadow-2xl"
          style={{ background: '#ffffff', border: '1px solid rgba(226,232,240,0.8)' }}>

          {/* Header */}
          <div className="px-8 pt-8 pb-6 text-center border-b border-slate-100" style={{ background: '#ffffff' }}>
            <div
              className="w-16 h-16 rounded-2xl flex items-center justify-center font-bold text-xl text-white mx-auto mb-4 shadow-lg"
          ...
          <div className="px-8 py-6 space-y-4" style={{ background: '#ffffff' }}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="p-4 rounded-2xl text-center" style={{ background: `${navy}08` }}>
              ...
              <div className="p-4 rounded-2xl text-center" style={{ background: 'rgba(5,150,105,0.08)' }}>
