...
import {
  ...
  LogOut, ShieldCheck, Clock, Building2, MapPin, Scan, Activity, AlertTriangle, BookOpen, Shield, Menu, X as XIcon,
} from 'lucide-react';
...
export default function SuperAdminDashboard({ onExit, adminData, user, onSwitchToStudent }: SuperAdminDashboardProps) {
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'users':       return <UserManagement isSuperAdmin={true} />;
      case 'temp':        return <TemporaryVisitorManagement isSuperAdmin={true} />;
      case 'reports':     return <ReportModule isSuperAdmin={true} />;
      case 'access':      return <AdminAccessManagement isSuperAdmin={true} />;
              ...
              <div className="space-y-0.5">
                {group.items.map(item => (
                  <button key={item.id} onClick={() => setActiveTab(item.id)}
                    ...
                    onMouseEnter={e => { if (activeTab !== item.id) { (e.currentTarget as HTMLElement).style.background = 'rgba(255,255,255,0.07)'; (e.currentTarget as HTMLElement).style.color = 'rgba(255,255,255,0.75)'; } }}
                    onMouseLeave={e => { if (activeTab !== item.id) { (e.currentTarget as HTMLElement).style.background = 'transparent'; (e.currentTarget as HTMLElement).style.color = 'rgba(255,255,255,0.45)'; } }}>
                    <item.icon size={17} />
